...
bool Runtime::execute(const Block&b,std::string&e){
 ...
 if(move && m.traori && has_tool_axis && m.motion!=MotionMode::ArcCW && m.motion!=MotionMode::ArcCCW){ ToolDefinition td{}; double tool_length=100.0; if(t && tools_.get(t,td)) tool_length=td.length; ToolPose pose{{next.X,next.Y,next.Z},tool_axis}; IKResult ik; if(!Kinematics::inverse_kinematics(pose,state_,tool_length,ik,e)) return false; next.A=ik.state.A; next.C=ik.state.C; } if(move){MotionPoint a{state_.X,state_.Y,state_.Z,state_.A,state_.C},z{next.X,next.Y,next.Z,next.A,next.C};bool ok;if(m.motion==MotionMode::ArcCW||m.motion==MotionMode::ArcCCW){if(m.plane!=17){e="G2/G3 currently supports G17 XY plane only";return false;}ok=ArcPlanner::plan(a,z,arc,m.motion==MotionMode::ArcCW,f,last_motion_,e);}else ok=MotionPlanner::plan(a,z,m.motion==MotionMode::Rapid,f,last_motion_,e);if(!ok)return false;}else{if(!Kinematics().validate(next,e))return false;last_motion_.clear();}
