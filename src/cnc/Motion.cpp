...
bool MotionPlanner::plan(const MotionPoint&s,const MotionPoint&t,bool,double feed,std::vector<MotionPoint>&out,std::string&e,int n){
 ...
 for(int i=0;i<=n;i++){double u=double(i)/n; MotionPoint p{s.X+(t.X-s.X)*u,s.Y+(t.Y-s.Y)*u,s.Z+(t.Z-s.Z)*u,s.A+(A-s.A)*u,s.C+(C-s.C)*u}; out.push_back(p); if(!Kinematics().validate({p.X,p.Y,p.Z,p.A,p.C},e))return false;} return true;
}
}