...
bool ArcPlanner::plan(const MotionPoint&s,const MotionPoint&t,const ArcDefinition&o,bool cw,double feed,std::vector<MotionPoint>&out,std::string&e,int n){
 if(n<2||feed<0||!std::isfinite(feed)){e="invalid arc parameters";return false;} const double cx=s.X+o.I,cy=s.Y+o.J,sx=s.X-cx,sy=s.Y-cy,tx=t.X-cx,ty=t.Y-cy,r0=std::hypot(sx,sy),r1=std::hypot(tx,ty); if(r0<1e-9||std::abs(r0-r1)>1e-5){e="invalid G2/G3 arc radius";return false;}
 constexpr double pi=3.14159265358979323846; double a0=std::atan2(sy,sx),da=std::atan2(ty,tx)-a0; if(cw){while(da>=0)da-=2*pi;}else{while(da<=0)da+=2*pi;} if(std::abs(da)<1e-12)da=cw?-2*pi:2*pi;
