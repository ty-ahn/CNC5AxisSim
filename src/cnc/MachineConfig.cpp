...
bool MachineConfig::load_json(const std::string& path,std::string& e){
 ...
 auto get=[&](const char*k,AxisLimit&a){double lo,hi;if(!num(s,std::string(k)+"_min",lo)||!num(s,std::string(k)+"_max",hi))return false;a.minimum=lo;a.maximum=hi;return true;};
