 ...
 std::ifstream f(path);if(!f){e="cannot open machine config";return false;}std::ostringstream ss;ss<<f.rdbuf();std::string raw=ss.str();JsonValue root;if(parse_json(raw,root,e)) {
  ...
  if(const auto* a=root.get("nodes");a&&a->arr()){
   ...
   for(const auto&v:*a->arr()){auto o=v.obj();if(!o)continue;MachineNode n;if(auto x=v.get("name");x&&x->str())n.name=*x->str();else{e="machine node name missing";return false;}
    ...
    if(auto x=v.get("min");x&&x->number())n.min=*x->number();if(auto x=v.get("max");x&&x->number())n.max=*x->number();if(auto x=v.get("limit");x&&x->object()){if(auto q=x->get("min");q&&q->number())n.min=*q->number();if(auto q=x->get("max");q&&q->number())n.max=*q->number();if(auto q=x->get("wrap");q&&std::holds_alternative<bool>(q->data))n.wrap=std::get<bool>(q->data);}
...
bool MachineConfig::save_json(const std::string& path,std::string&e)const{
 ...
 for(size_t i=0;i<nodes_.size();++i){const auto&n=nodes_[i];const char*t=n.type==MachineNodeType::LinearAxis?"linear":n.type==MachineNodeType::RotaryAxis?"rotary":n.type==MachineNodeType::Tool?"tool":n.type==MachineNodeType::Holder?"holder":"fixture";f<<"      {\n        \"name\": \""<<json_escape(n.name)<<"\", \"type\": \""<<t<<"\",\n        \"parent\": "<<(n.parent.empty()?"null":"\""+json_escape(n.parent)+"\"")<<",\n        \"axis\": ["<<n.axis.x<<", "<<n.axis.y<<", "<<n.axis.z<<"],\n        \"pivot\": ["<<n.pivot.x<<", "<<n.pivot.y<<", "<<n.pivot.z<<"],\n        \"limit\": {\"min\": "<<n.min<<", \"max\": "<<n.max<<", \"wrap\": "<<(n.wrap?"true":"false")<<"},\n        \"mesh\": \""<<json_escape(n.mesh)<<"\", \"collision\": "<<(n.collision_enabled?"true":"false")<<"\n      }"<<(i+1<nodes_.size()?",":"")<<"\n";}
 f<<"    ]\n  }\n}\n";return true;
}nclude "MachineConfig.h"
#include "MachineJson.h"
#include <fstream>
#include <sstream>
#include <regex>
