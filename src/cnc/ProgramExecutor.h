...
class ProgramExecutor {
 Runtime* runtime_{}; std::vector<Block> blocks_; size_t index_{0}; ExecutionState state_{ExecutionState::Idle}; std::string error_; std::unordered_map<int,size_t> labels_; std::unordered_map<int,std::vector<Block>> subprograms_; struct Frame{int program{}; size_t return_index{};}; std::vector<Frame> stack_; size_t steps_{0}; size_t max_steps_{1000000}; int program_{0};
public:
 explicit ProgramExecutor(Runtime& r):runtime_(&r){}
 void load(const std::vector<Block>& b){blocks_=b;subprograms_.clear();stack_.clear();program_=0;index_=0;steps_=0;error_.clear();labels_.clear();state_=ExecutionState::Idle; for(size_t i=0;i<blocks_.size();++i) if(blocks_[i].number>=0) labels_[blocks_[i].number]=i;}
 ...
 bool step(){if(++steps_>max_steps_){error_="execution step limit exceeded";state_=ExecutionState::Error;return false;} if(state_==ExecutionState::Held||state_==ExecutionState::Error||index_>=blocks_.size()){if(index_>=blocks_.size())state_=ExecutionState::Completed;return false;} if(!runtime_->execute(blocks_[index_],error_)){state_=runtime_->feed_hold()?ExecutionState::Held:ExecutionState::Error;return false;}
  ...
  if(exu.find("RET")!=std::string::npos && exu.find("RETURN")!=std::string::npos){ if(stack_.empty()){error_="RET without CALL";state_=ExecutionState::Error;return false;} auto fr=stack_.back(); stack_.pop_back(); if(fr.program==0){program_=0; blocks_=blocks_;} else {program_=fr.program;} index_=fr.return_index; return true; }
