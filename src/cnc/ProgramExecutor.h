...
class ProgramExecutor {
 static void index_labels(const std::vector<Block>& b,std::unordered_map<int,size_t>& out){out.clear();for(size_t i=0;i<b.size();++i){if(b[i].number>=0)out[b[i].number]=i;std::string u=b[i].source;for(char& ch:u)ch=char(std::toupper((unsigned char)ch));auto p=u.find("LBL");if(p!=std::string::npos){p+=3;while(p<u.size()&&std::isspace((unsigned char)u[p]))++p;int n=std::atoi(u.c_str()+p);if(n>0)out[n]=i;}}}
...
public:
 ...
 void load(const std::vector<Block>& b){blocks_=b; main_blocks_=b; subprograms_.clear();stack_.clear();program_=0;index_=0;steps_=0;error_.clear();labels_.clear();state_=ExecutionState::Idle; index_labels(blocks_,labels_);}
 ...
 bool step(){if(state_==ExecutionState::Held||state_==ExecutionState::Error||index_>=blocks_.size()){if(index_>=blocks_.size())state_=ExecutionState::Completed;return false;} if(++steps_>max_steps_){error_="execution step limit exceeded";state_=ExecutionState::Error;return false;} const auto& pre=blocks_[index_].source; std::string preu=pre; for(char& ch:preu)ch=char(std::toupper((unsigned char)ch));
  size_t rp=preu.find('R'); if(rp!=std::string::npos){size_t eq=preu.find('=',rp); if(eq!=std::string::npos){size_t q=rp+1;while(q<preu.size()&&std::isdigit((unsigned char)preu[q]))++q;if(q>rp+1){int rn=std::atoi(preu.c_str()+rp+1);std::string rhs=pre.substr(eq+1);double rv=0;if(!Expression::evaluate(rhs,runtime_->variables(),rv,error_)){state_=ExecutionState::Error;return false;}runtime_->variables().set(rn,rv);}}} if(!runtime_->execute(blocks_[index_],error_)){state_=runtime_->feed_hold()?ExecutionState::Held:ExecutionState::Error;return false;}
  const auto& executed=blocks_[index_].source; std::string exu=executed; for(char& ch:exu) ch=char(std::toupper((unsigned char)ch));
  if(exu.find(" RET")!=std::string::npos || exu.rfind("RET",0)==0){ if(stack_.empty()){error_="RET without CALL";state_=ExecutionState::Error;return false;} auto fr=stack_.back(); stack_.pop_back(); if(fr.program==0){program_=0; blocks_=main_blocks_;} else {program_=fr.program;} labels_.clear(); index_labels(blocks_,labels_); index_=fr.return_index; return true; }
  ...
  if(pos!=std::string::npos){ size_t p=pos+5; while(p<src.size()&&std::isspace((unsigned char)src[p]))++p; int n=std::atoi(src.c_str()+p); auto it=labels_.find(n); if(it==labels_.end()){error_="label not found";state_=ExecutionState::Error;return false;} index_=it->second; return true; } ++index_;if(index_>=blocks_.size())state_=ExecutionState::Completed;return true;}
