...
int main(){
 ...
 assert(std::abs(cnc::Kinematics::unwrap(359,1)-361)<1e-9); cnc::Kinematics kk({{-100,100,false},{-100,100,false},{-100,100,false},{-90,90,false},{-180,180,true},10,20}); cnc::MachineState ks{0,0,0,30,40}; auto tcp=kk.tcp_from_machine(ks,50); cnc::ToolPose kp{tcp,kk.configured_axis_from_ac(30,40)}; cnc::IKResult ki; assert(kk.inverse_kinematics(kp,ks,50,ki,e)); assert(std::abs(ki.position_error)<1e-7); cnc::ToolPose target{tcp,kk.configured_axis_from_ac(70,120)}; std::vector<cnc::MachineState> path; assert(kk.orientation_move(target,ks,50,path,e,12)); assert(path.size()==13); for(const auto& q:path){auto p=kk.tcp_from_machine(q,50); assert(std::sqrt((p.x-tcp.x)*(p.x-tcp.x)+(p.y-tcp.y)*(p.y-tcp.y)+(p.z-tcp.z)*(p.z-tcp.z))<1e-7);}
