...
int main(){
 ...
 cnc::Runtime cyc; assert(cyc.execute(cnc::Parser::parse("N10 CYCLE800(0,0,0,0,0,0,0,30,0,45,0,0,0,0)"),e)); assert(cyc.kinematics().cycle800_active()); assert(cyc.execute(cnc::Parser::parse("N20 CYCLE800()"),e)); assert(!cyc.kinematics().cycle800_active());
