...
int main(){
 ...
 run("N90 G55 M5"); run("N100 G81 X20 Y20 Z-10 R5 F100"); assert(std::abs(r.state().Z)<1e-9&&r.last_motion().size()>1); run("N105 G99 G81 X30 Y30 Z-10 R5 F100"); assert(std::abs(r.state().Z-5)<1e-9); run("N110 G80"); assert(r.modal().work_offset==55&&r.rpm()==0);
